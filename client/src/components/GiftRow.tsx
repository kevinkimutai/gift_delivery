          ...
          <>
            ...
            <div className="flex justify-start items-start bg-white p-4 pb-7 w-full overflow-x-scroll scrollbar-hide">
              {data?.gifts.map((item: any) => (
                <div
                  key={item.id}
                  className="flex justify-between flex-col border-2 shadow p-2 min-w-[10rem] m-3 h-[15rem] hover:scale-110 hover:shadow-sm cursor-pointer transition duration-700 ease-in-out"
                >
                  <img
                    src={item.image}
                    alt={item.name}
